...
module.exports = {
	...
	langs: {
		vi: {
			...
			// Bug Fix
			rulesNotExistMove: "Không tồn tại nội quy thứ %1",
		...
		en: {
			...
			// Bug Fix
			rulesNotExistMove: "Rule number %1 does not exist",
...
onStart: async function ({ role, args, message, event, threadsData, getLang, commandName }) {
		...
		if (!type) {
			let i = 1;
			const msg = rulesOfThread.reduce(
				(text, rules) => text += `${i++}. ${rules}\n`,
				""
			);
			...
			if (!rulesOfThread[stt - 1]) {
				return message.reply(
					`${getLang("rulesNotExist", stt)}, ${
						totalRules == 0
							? getLang("noRules", global.GoatBot.config.prefix)
							: getLang("numberRules", totalRules)
					}`
				);
			}
			...
			if (!args[1] || isNaN(args[1]))
				return message.reply(
					getLang("invalidNumberDelete")
				);

			const rulesDel =
				rulesOfThread[parseInt(args[1]) - 1];
			...
			if (role < 1)
				return message.reply(
					getLang("noPermissionRemove")
				);

			message.reply(
				getLang("confirmRemove"),
				(err, info) => {
					if (err)
						return;
